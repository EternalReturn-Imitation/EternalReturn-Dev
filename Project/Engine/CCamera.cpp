...
{
	// ¸¸¾à¿¡ Collider2D°¡ ¾ø°Å³ª Rect¸ð¾çÀÌ ¾Æ´Ñ °æ¿ì return
...
{
	// ¸¸¾à¿¡ Collider3D°¡ ¾ø°Å³ª Cube¸ð¾çÀÌ ¾Æ´Ñ °æ¿ì return
	...
	Vec3 arrLocal[6][3] =
	{
		...
		{Vec3(-0.5f, -0.5f, 0.5f),  Vec3(0.5f, -0.5f, 0.5f),  Vec3(-0.5f, 0.5f, 0.5f)},  // µÞ¸é
		{Vec3(-0.5f, 0.5f, -0.5f),  Vec3(-0.5f, -0.5f, -0.5f),Vec3(-0.5f, 0.5f, 0.5f)},  // ¿ÞÂÊ¸é
...
IntersectResult CCamera::IsCollidingBtwRaySphere(tRay& _ray, CGameObject* _Object)
{
	...
	// ¸¸¾à¿¡ Collider3D°¡ ¾ø°Å³ª Sphere¸ð¾çÀÌ ¾Æ´Ñ °æ¿ì return
	...
	// m º¤ÅÍÀÇ Á¦°ö°ú ±¸ÀÇ ¹ÝÁö¸§ÀÇ Á¦°öÀÇ Â÷¸¦ °è»êÇÕ´Ï´Ù.
	float c = (m.x * m.x + m.y * m.y + m.z * m.z) - SphereRadius * SphereRadius;

	// ·¹ÀÌÀÇ ¿øÁ¡ÀÌ ±¸ÀÇ ¿ÜºÎ¿¡ ÀÖ°í (c > 0), ·¹ÀÌ°¡ ±¸¸¦ ÇâÇÏ°í ÀÖÁö ¾ÊÀ¸¸é (b > 0) false¸¦ ¹ÝÈ¯ÇÕ´Ï´Ù.
	if (c > 0.0f && b > 0.0f)
	{
		result.vCrossPoint = Vec3(0.f, 0.f, 0.f);
		result.fResult = 0.f;
	...
	// ÆÇº°½ÄÀÌ À½¼öÀÌ¸é ·¹ÀÌ°¡ ±¸¸¦ ³õÄ£ °ÍÀÌ¹Ç·Î false¸¦ ¹ÝÈ¯ÇÕ´Ï´Ù.
	if (discr < 0.0f)
	{
		result.vCrossPoint = Vec3(0.f, 0.f, 0.f);
		result.fResult = 0.f;
	...
	//m_LayMinDistance = depth;  // ÇöÀç µð½ºÅÄ½º °ªÀ¸·Î ¾÷µ¥ÀÌÆ®
